...
{
	lua_pushstring(m_state, key);
	lua_gettable(m_state, -2); // 从 Terrain 表中获取函数  
	if (!lua_isfunction(m_state, -1)) {
		// 错误处理，convertLatLonToMeters 不是函数...  
		printf("%s不是函数\n", key);
		lua_pop(m_state, 2); // 弹出函数和 Terrain 表
		return false;
	}
	return true;
}
void LuaVM::convertLatLonToMeters(double lat, double lon, double& x, double& y)
{
	...
	if (!luaOkay(lua_pcall(m_state, 2, 2, 0))) {
		// 错误处理，函数调用失败...  
		lua_pop(m_state, 2); // 弹出错误消息和 Terrain 表  
		return;
	}
	// 检查结果  
	...
	// 清理栈  
	lua_pop(m_state, 3); // 弹出两个返回值和 Terrain 表
}
void LuaVM::convertMetersToLatLon(double x, double y, double& lat, double& lon)
{
	...
	if (!luaOkay(lua_pcall(m_state, 2, 2, 0))) {
		// 错误处理，函数调用失败...  
		lua_pop(m_state, 2); // 弹出错误消息和 Terrain 表  
		printf("convertMetersToLatLon调用失败\n");
		return;
	}
	// 检查结果  
	if (lua_isnumber(m_state, -2) && lua_isnumber(m_state, -1)) {
		...
		// 错误处理，返回的不是两个数字... 
		printf("convertMetersToLatLon返回的不是2个数字\n");
	}
	// 清理栈  
	lua_pop(m_state, 3); // 弹出两个返回值和 Terrain 表
}
void LuaVM::getTerrainTbl()
{
	...
	lua_setfield(m_state, -2, "cpath"); // 设置新的 cpath  
	lua_pop(m_state, 1); // 弹出 package 表
	// 执行 Lua 代码字符串  
