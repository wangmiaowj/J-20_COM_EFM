...
public:
	EDPARAM()
	{
		...
		devicesClass[RWR] = nullptr;
		devicesClass[UFCP] = nullptr;
	}
	~EDPARAM()
	{
		delete tacanPtr;
	...
	void* initDevice(void** devicesTbl, int idx, const char* deviceName)
	{
		void* device = devicesTbl[idx];
		if (!device) {
			printf("%s not exit\n", deviceName);
			return nullptr;
		}
	...
	void tryInit()
	{
		...
		if (devicesClass[RADAR_RAW] == nullptr)
		{
			devicesClass[RADAR_RAW] = initDevice(deviceTable, devices::RADAR_RAW, "radarDevice");
		}
		if (devicesClass[UHF_RADIO] == nullptr)
		{
			devicesClass[UHF_RADIO] = initDevice(deviceTable, devices::UHF_RADIO, "uhfDevice");
		}
		if (devicesClass[VHF_AM_RADIO] == nullptr)
		{
			devicesClass[VHF_AM_RADIO] = initDevice(deviceTable, devices::VHF_AM_RADIO, "vhfDevice");
		}
		if (devicesClass[VHF_FM_RADIO] == nullptr)
		{
			devicesClass[VHF_FM_RADIO] = initDevice(deviceTable, devices::VHF_FM_RADIO, "vhfDevice");
		}
		if (devicesClass[INTERCOM] == nullptr)
		{
			devicesClass[INTERCOM] = initDevice(deviceTable, devices::INTERCOM, "intercomDevice");
		}
		if (devicesClass[RWR] == nullptr)
		{
			devicesClass[RWR] = initDevice(deviceTable, devices::RWR, "rwrDevice");
		}
		if (devicesClass[CLOCK] == nullptr)
		{
			devicesClass[CLOCK] = initDevice(deviceTable, devices::CLOCK, "avA11ClockDevice");
		}
		if (devicesClass[UFCP] == nullptr)
		{
			devicesClass[UFCP] = initDevice(deviceTable, devices::UFCP, "avLuaDevice");
		}
		/*void* elec_h = getParamHandle("ELEC_PTR");
		void* weapon_h = getParamHandle("WEAPON_PTR");
		void* ils_h = getParamHandle("ILS_PTR");
		void* uhf1_h = getParamHandle("UHF1_PTR");
		void* uhf2_h = getParamHandle("UHF2_PTR");
		void* intercom_h = getParamHandle("INTERCOM_PTR");
		void* radar_h = getParamHandle("RADAR_PTR");
		void* rwr_h = getParamHandle("RWR_PTR");
		devicesClass[ELECTRIC_SYSTEM] = getPointer(t);
		devicesClass[WEAPON_SYSTEM] = getPointer(weapon_h);
		devicesClass[ILS] = getPointer(ils_h);
		devicesClass[RADAR_RAW] = getPointer(radar_h);
		devicesClass[UHF_RADIO] = getPointer(uhf1_h);
		devicesClass[VHF_AM_RADIO] = getPointer(uhf2_h);
		devicesClass[INTERCOM] = getPointer(intercom_h);
		devicesClass[RWR] = getPointer(rwr_h);
		printf("weaponDevice: %p\n", devicesClass[WEAPON_SYSTEM]);
		printf("ilsDevice: %p\n", devicesClass[ILS]);
		printf("radarDevice: %p\n", devicesClass[RADAR_RAW]);
		printf("uhf1Device: %p\n", devicesClass[UHF_RADIO]);
		printf("uhf2Device: %p\n", devicesClass[VHF_AM_RADIO]);
		printf("intercomDevice: %p\n", devicesClass[INTERCOM]);
		printf("rwrDevice: %p\n", devicesClass[RWR]);*/
	}
	...
	{
		uintptr_t ptr = NULL;
		const char* buffer = getParamString(handle, 20);
		printf("%s\n", buffer);
		//printf( "%s\n", buffer );
		...
		{
			printf("Pointer could not be found from string: %s\n", buffer);
		}
	...
	inline void VHF_connect_electric()
	{
		if (devicesClass[VHF_AM_RADIO] == nullptr)
		{
			throw new MyException("VHF_AM没有初始化");
		...
		if (wire != nullptr)
		{
			ed_param_api.pfn_vhf_connect_electric(devicesClass[VHF_AM_RADIO], wire);
			ed_param_api.pfn_vhf_connect_electric(devicesClass[VHF_FM_RADIO], wire);
		}
		else
		{
			throw new MyException("getDCbus无法获取直流电");
