...
public:
	...
	void update(double dt, State& state, Quaternion& quaternion)
	{
...
public:
	...
	void update(double dt, State& state, double timestamp, Quaternion& quaternion)
	{
		if (ctx == snd.WORLD_CONTEXT)
		{
			...
			params->orientation[2] = quaternion.z;
			params->orientation[3] = quaternion.w;
			params->velocity[0] = state.m_localSpeed.x;
			params->velocity[1] = state.m_localSpeed.y;
			params->velocity[2] = state.m_localSpeed.z;
		}
		for (size_t i = 0; i < sndSources.size(); i++)
		{
			sndSources[i]->update(dt, state, quaternion);
		}
		params->timestamp = timestamp;
