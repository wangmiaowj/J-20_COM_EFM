...
void ed_fm_set_draw_args(EdDrawArgument* drawargs, size_t size)
{
	...
	drawargs[0].f = s_airframe->getGearNPosition();//Nosewheel Position前起落架
	drawargs[2].f = s_airframe->getNoseWheelAngle();//Nosewheel Angle +/- 60? 鼻轮转向角度
	drawargs[3].f = s_airframe->getGearRPosition();//Right Gear Position 右起落架
	drawargs[5].f = s_airframe->getGearLPosition();//Left Gear Position 左起落架
	if (s_airframe->fcsIsBiting())
	{
		drawargs[9].f = s_airframe->getBit_flapR();//right flap Versuch 右襟翼
		drawargs[10].f = s_airframe->getBit_flapL();//left flap Versuch 左襟翼
		drawargs[11].f = s_airframe->getBit_aileronR();//right aileron 右副翼
		drawargs[12].f = -s_airframe->getBit_aileronL();//left Aileron 左副翼
		drawargs[13].f = s_airframe->getBit_leflapR();//右前缘襟翼
		drawargs[14].f = s_airframe->getBit_leflapL();//左前缘襟翼
		drawargs[15].f = s_airframe->getBit_canardR(); //right elevator ist standard f黵 ein Leitwerk 右鸭翼
		drawargs[16].f = s_airframe->getBit_canardL();//left elevator 左鸭翼
		drawargs[17].f = -s_airframe->getBit_rudderR(); //rudder 右垂尾
		drawargs[18].f = -s_airframe->getBit_rudderL();//rudder nr. 2 左垂尾
		drawargs[21].f = s_airframe->getBit_airBrake(); //扰流板
		drawargs[181].f = s_airframe->getBit_canardBrake();//鸭翼扰流板
	}
	else
	{
		drawargs[9].f = s_airframe->getFlapsPosition();//right flap Versuch 右襟翼
		drawargs[10].f = s_airframe->getFlapsPosition();//left flap Versuch 左襟翼
		drawargs[11].f = s_airframe->getAileron();//right aileron 右副翼
		drawargs[12].f = -s_airframe->getAileron();//left Aileron 左副翼
		drawargs[13].f = s_airframe->getLEFlapPosition();//右前缘襟翼
		drawargs[14].f = s_airframe->getLEFlapPosition();//左前缘襟翼
		drawargs[15].f = s_state->getAoaRate(); //right elevator ist standard f黵 ein Leitwerk 右鸭翼
		drawargs[16].f = s_state->getAoaRate();//left elevator 左鸭翼
		drawargs[17].f = -s_airframe->getRudder(); //rudder 右垂尾
		drawargs[18].f = -s_airframe->getRudder();//rudder nr. 2 左垂尾
		drawargs[21].f = s_airframe->getSpeedBrakePosition(); //扰流板
		drawargs[181].f = s_airframe->getSpeedCanardBrakePosition();//鸭翼扰流板
	}
	drawargs[22].f = s_airframe->getRefuelingDoor();// 加油管
	drawargs[23].f = s_flightModel->getWheelChockStatus();
	drawargs[26].f = s_airframe->getBayDoorsPosition();//BayDoorPosition 0.0 = zu 1.0 = auf AI弹仓
	drawargs[28].f = s_engine->updateBurner2();//Burner Stage 1 and 0 for right engine 右发动机燃烧室动画
	drawargs[29].f = s_engine->updateBurner();//Burner Stage 1 and 0 for right engine 左发动机燃烧室动画
	drawargs[35].f = s_airframe->brkChutePosition();//减速伞释放
	drawargs[36].f = s_airframe->getChutePositionY();//slew chute in Y-Axis (-1 to +1) 减速伞Y轴摆动
	drawargs[37].f = s_airframe->getChutePositionZ();//slew chute in Z-Axis (-1 to +1) 减速伞X轴摆动
	drawargs[85].f = s_airframe->getLaunchBarPos();//aktuell noch ohne Anbindung an die Actuators, weil wir auch eh nix zum anziegne haben... 弹射杆
	drawargs[89].f = s_airframe->getNozzlePosition();//Engine Nozzle Stage 1 - 0 - 1 右喷口动画
